...
<div>
    ...
    <main class="my-2">
        <div class="container mx-auto px-6">
            <h3 class="text-gray-700 text-2xl font-medium">Ideas Box</h3>
            <div class="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 mt-6">
                @foreach ($ideas as $idea)
                    
                
                <div class="w-full max-w-sm mx-auto rounded-md shadow-md overflow-hidden">
                    <div class="flex items-end justify-end h-56 w-full bg-cover" style="background-image: url('{{$idea->image}}')">
                        <button class="p-2 rounded-full bg-blue-600 text-white mx-5 -mb-4 hover:bg-blue-500 focus:outline-none focus:bg-blue-500" wire:click="subscribe({{$idea->id}})">
                    ...
                    <div class="px-5 py-3">
                        <h3 class="text-gray-700 uppercase">{{$idea->title}}</h3>
                        <span class="text-gray-500 mt-2">{{$idea->price}}XAF</span>
                    </div>
                </div>
                @endforeach
